/**
 * DZ-RentIt — EditItemPage
 * =========================
 * 
 * Multi-step form for editing an existing item listing.
 * Pre-populates form data from the current item.
 * Uses useItems.updateItem for persistence.
 * 
 * ARCHITECTURE: Same step structure as AddItemPage to maintain UX consistency.
 * Business logic delegated to useItems hook — no direct API calls in JSX.
 */
...
export default function EditItemPage() {
  ...
  const { updateItem, loading } = useItems();
  const [currentStep, setCurrentStep] = useState(1);
  const [itemLoaded, setItemLoaded] = useState(false);

  // Find item from mock data (in production, would fetch from API)
  const existingItem = mockItems.find((i) => i.id === id);
  ...
  // Pre-populate form when item is found
  useEffect(() => {
    if (existingItem && !itemLoaded) {
      setFormData({
        title: existingItem.title || '',
        description: existingItem.description || '',
        category: existingItem.category || '',
        photos: existingItem.images || [],
        price: String(existingItem.price || ''),
        deposit: String(existingItem.deposit || ''),
        condition: existingItem.condition || 'excellent',
        availableDays: existingItem.availableDays || ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
      });
      setItemLoaded(true);
    }
  }, [existingItem, itemLoaded]);
      ...
      <PageTransition>
        <div className="min-h-screen bg-gray-50 dark:bg-dark flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4">The item you're trying to edit doesn't exist.</p>
          ...
          <div className="max-w-2xl mx-auto">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Update the details of <span className="font-medium text-gray-700 dark:text-gray-300">"{existingItem.title}"</span>
              </p>
            </div>
            ...
            <div className="card p-6 md:p-8">
              <AnimatePresence mode="wait">
                ...
                {currentStep === 3 && (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Update your photos (max 6). First image is the cover.</p>
                ...
                {currentStep === 4 && (
                  <motion.div
                    ...
                    className="space-y-5"
                  >
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Update Pricing</h2>
                    <Input
                ...
                {currentStep === 5 && (
                  ...
                  >
                    ...
                    <div className="p-5 rounded-xl bg-gray-50 dark:bg-dark-50 border border-gray-200 dark:border-dark-border space-y-3">
                      <h3 className="font-semibold text-gray-900 dark:text-white">Updated Listing Summary</h3>
              ...
              <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-100 dark:border-dark-border">
                ...
                >
                  {currentStep === 5 ? (loading ? 'Saving...' : 'Save Changes') : 'Continue'}
                  {currentStep < 5 && <ArrowRight className="w-4 h-4 ml-1" />}
                  {currentStep === 5 && !loading && <Check className="w-4 h-4 ml-1" />}
                </Button>
              </div>
            </div>
          </div>
